...
def make_hamiltonian(Nrep,Nbond,Ns,list_site1,list_site2,list_state):
#
...
##    Ham = np.zeros((Nrep,Nrep),dtype=np.float64)
    @jit(nopython=True)
    def get_vec(vec):
        vecnew = np.zeros(Nrep,dtype=np.float64)
        for a in range(Nrep):
            sa = list_state[a]
...
#    Ham = make_hamiltonian(Nrep,elemnt,listki,loc)
    get_vec = make_hamiltonian(Nrep,Nbond,Ns,list_site1,list_site2,list_state)
