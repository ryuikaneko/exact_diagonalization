...
def make_hamiltonian(Jxx,Jzz,Nbond,list_site1,list_site2,Nrep,list_state,list_sqrtnorm,Lx,Ly,Ns,momkx,momky,expk):
...
#                        loc[i*Nrep+a] = b
                        vecnew[a] += vec[b]*wght*list_sqrtnorm[b]/list_sqrtnorm[a]*expk[expx,expy]
...
#        print(i,list_state[i],num2bit(list_state[i],Ns),list_R[i],list_F2[i],list_sqrtnorm[i])
    ...
    start = time.time()
    Jxx, Jzz, list_site1, list_site2, Nbond = make_lattice(Lx,Ly,J1,J2)
    Jxx = np.array(Jxx)
    Jzz = np.array(Jzz)
...
#    Ham = make_hamiltonian(Nrep,elemnt,listki,loc)
    get_vec = make_hamiltonian(Jxx,Jzz,Nbond,list_site1,list_site2,Nrep,list_state,list_sqrtnorm,Lx,Ly,Ns,momkx,momky,expk)
