...
function get_ja_plus_jb(ii,irght,ilft,ihfbit,list_ja,list_jb)
    ia = ii & irght
    ib = div((ii & ilft),ihfbit)
    @inbounds ja = list_ja[ia+1]
    @inbounds jb = list_jb[ib+1]
    return ja+jb
end
...
#    end
    function get_vec!(vecnew::AbstractVector,vec::AbstractVector)
        ...
        @simd for i in 0:Nhilbert-1
            @inbounds vecnew[i+1] = 0.0
        end
        for k in 0:Nint-1
            @inbounds isite1 = list_isite1[k+1]
            @inbounds isite2 = list_isite2[k+1]
            is1 = 1<<isite1
            is2 = 1<<isite2
            is12 = is1 + is2
            @inbounds wght = 2.0*Jxx[k+1]
            @inbounds diag = Jzz[k+1]
            @simd for i in 0:Nhilbert-1
                @inbounds ii = list_1[i+1]
                ibit = ii & is12
#                loc[Nint*Nhilbert+i+1] = i+1
                if (ibit==0 || ibit==is12)
                    @inbounds vecnew[i+1] += +diag*vec[i+1]
...
#                    elemnt[Nint*Nhilbert+i+1] -= diag
                    ...
                    @inbounds vecnew[i+1] += vec[newcfg+1]*wght
...
#    @time Ham = make_hamiltonian(Jxx,Jzz,list_isite1,list_isite2,N,Nint,Nhilbert,irght,ilft,ihfbit,list_1,list_ja,list_jb)
    @time get_vec_LM = make_hamiltonian(Jxx,Jzz,list_isite1,list_isite2,N,Nint,Nhilbert,irght,ilft,ihfbit,list_1,list_ja,list_jb)
#    println("")
#    println("get_vec_LM")
#    println(get_vec_LM)
