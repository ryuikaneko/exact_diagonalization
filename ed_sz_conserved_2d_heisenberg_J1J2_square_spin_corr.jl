...
#    list_isite2 = []
    ...
    for iy in 0:Ly-1
        for ix in 0:Lx-1
            site1 = ix + Lx*iy
            site1x = mod((ix+1),Lx) + Lx*iy
            site1y = ix + Lx*(mod((iy+1),Ly))
            site1xpy = mod((ix+1),Lx) + Lx*(mod((iy+1),Ly))
            site1xmy = mod((ix+1),Lx) + Lx*(mod((iy-1+Ly),Ly))
#
            push!(list_isite1,site1)
            push!(list_isite2,site1x)
            push!(Jxx,J1)
            push!(Jzz,J1)
            Nint += 1
#
            push!(list_isite1,site1)
            push!(list_isite2,site1y)
            push!(Jxx,J1)
            push!(Jzz,J1)
            Nint += 1
#
            push!(list_isite1,site1)
            push!(list_isite2,site1xpy)
            push!(Jxx,J2)
            push!(Jzz,J2)
            Nint += 1
#
            push!(list_isite1,site1)
            push!(list_isite2,site1xmy)
            push!(Jxx,J2)
            push!(Jzz,J2)
            Nint += 1
        end
...
#    J2 = 0.0
    ...
    @time Jxx, Jzz, list_isite1, list_isite2, Nint = make_lattice(Lx,Ly,J1,J2)
    println(Jxx)
    println(Jzz)
    println(list_isite1)
    println(list_isite2)
